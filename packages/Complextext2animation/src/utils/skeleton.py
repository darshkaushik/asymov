...
class Skeleton:
    ...
    def forward_kinematics(self, rotations, root_positions):
        ...
        self._offsets *= 1.0/np.abs(self._offsets).max()
        expanded_offsets = self._offsets.expand(rotations.shape[0], rotations.shape[1],
                                                   self._offsets.shape[0], self._offsets.shape[1])#.to(device='cuda')
    ...
    def debug_forward_kinematics(self, rotations, root_positions):
        """
        Perform forward kinematics using the given trajectory and local rotations.
        Arguments (where N = batch size, L = sequence length, J = number of joints):
         -- rotations: (N, L, J, 4) tensor of unit quaternions describing the local rotations of each joint.
        ...
        self._offsets = self._offsets[..., [0, 2, 1]]
        ...
        for i in range(self._offsets.shape[0]):
            ...
            # TODO: Remove debug statements
            print('Joint # {0}: {1}, with parent: {2}'\
                                .format(i, self._joints[i], self._parents[i]))

            if self._parents[i] == -1:
                # e = test_angles.q2e(rotations[0, 0, 0])
                # print('Original angle = ', e)

                positions_world.append(root_positions)
                rotations_world.append(rotations[:, :, 0])
            else:
                '''
                # TODO: Revert to original. AC's experiment: Rotate wrt _current_ joint angle.
                # -----------------------------------------
                rotations_world.append(qmul(rotations_world[self._parents[i]], rotations[:, :, i]))
                positions_world.append(qrot(rotations_world[i], expanded_offsets[:, :, i]) \
                                       + positions_world[self._parents[i]])
                '''
                # Original code
                positions_world.append(qrot(rotations_world[self._parents[i]], expanded_offsets[:, :, i]) \
                ...
                if self._has_children[i]:
                    ...
                    '''
                    if i == 6:  # LE
                        print('Offset (bone) = ', expanded_offsets[0,0,i])
                        e = test_angles.q2e(rotations[0, 0, i])
                        print('Local angle in Radians= ', e)
                        e = test_angles.q2e(rotations[0, 0, i], True)
                        print('Local angle in degrees = ', e)
                        pdb.set_trace()
                    '''
                else:
                    # This joint is a terminal node -> it would be useless to compute the transformation
