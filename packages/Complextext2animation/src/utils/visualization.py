...
def render_animation(figures, skeleton, fps, num_frames, output='out.mp4', bitrate=1000, audios=[], start_time=0, figsize=(4, 4), userStudy=0, history=0, history_offset=1, suptitle=''):
    ...
    def init_func(fig, figures, skeleton, figsize, fps):
        ...
        y = 1
        z = 2
        # Heuristic that works well with many skeletons
        ...
        for fig_num, values in enumerate(figures):
            ...
            if kind == 'skeleton':
                ...
                avg_segment_length = np.mean(np.linalg.norm(
                    np.diff(trajectory, axis=0), axis=1)) + 1e-3
                ...
                height_offset = np.min([np.min(data[:, :, 1])
                                        for data in datas])  # Min height
    ...
    def update(frame, fig, figures, skeleton, skeleton_parents, radius, camera_pos, trajectory, draw_offset, spline_line, x, y, z, axes):
        ...
        for fig_num, values in enumerate(zip(initialized, figures, axes, liness)):
            ...
            if kind == 'skeleton':
                ...
                # color_list = [['black', 'red'], ['orange', 'purple']]
                color_list = [['purple', 'blue'], ['black', 'purple']]
                #ax = fig.add_subplot(pos[0], pos[1], pos[2], projection='3d')
                ax.set_xlim3d([-radius/2 + camera_pos[frame, 0],
                               radius/2 + camera_pos[frame, 0]])
                ax.set_ylim3d([-radius/2 + camera_pos[frame, 1],
                               radius/2 + camera_pos[frame, 1]])

                positions_world = [[data[fr] for fr in range(
                    frame, frame-history*history_offset-1, -history_offset)] for data in datas]

                # AC Edit: Instead of looping over positions, maybe loop over
                # skeleton_parents? This automatically ignores "extra" joints
                # for i in range(positions_world[0][0].shape[0]):
                for i, _ in enumerate(skeleton_parents):
                    ...
                    if not Initialized:
                        for count in range(len(datas)):
                            ...
                            for hist, alpha in zip(range(history+1), alpha_range):
                                col = color_list[count][0] if i in skeleton.joints_right(
                                ) else color_list[count][1]  # As in audio cables :)
                        ...
                        for count in range(len(datas)):
                            for hist in range(history, -1, -1):
                                try:
                                    liness[fig_num][count][hist][i-1][0].set_xdata(
                                        [positions_world[count][hist][i, x], positions_world[count][hist][skeleton_parents[i], x]])
                                except:
                                    pdb.set_trace()
                                liness[fig_num][count][hist][i-1][0].set_ydata(
                                    [positions_world[count][hist][i, y], positions_world[count][hist][skeleton_parents[i], y]])
                                liness[fig_num][count][hist][i-1][0].set_3d_properties(
                                    [positions_world[count][hist][i, z], positions_world[count][hist][skeleton_parents[i], z]], zdir='y')
            ...
            if kind == 'graph':
                if frame <= fps/2:
                    ...
                    liness[fig_num][0].set_data(
                        range(0, frame), datas[0:frame])
                else:
                    ax.set_xlim([frame-fps/2, frame+fps/2])
                    liness[fig_num][0].set_data(
                        range(int(frame-fps/2), frame), datas[int(frame-fps/2):frame])

            if kind == 'highlight':
                outputs, mask = datas
                inv_mask = 1-mask
                ln = liness[fig_num][0]
                non_zero_indices = [idx for idx in range(
                    outputs[0].shape[1]) if not mask[frame, idx] == 0]
                ln.set_xdata(outputs[0][frame, non_zero_indices, x])
                ln.set_ydata(outputs[0][frame, non_zero_indices, y])
                ln.set_3d_properties(
                    outputs[0][0, non_zero_indices, z], zdir='y')
    ...
    anim = FuncAnimation(fig, update, frames=np.arange(history*history_offset, num_frames),
                         interval=1000/fps, repeat=False, fargs=init_func(fig, figures, skeleton, figsize, fps))
