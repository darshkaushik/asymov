...
def render_all(args, exp_num):
    ...
    def is_file_feat_type(fp, feat_type):
        '''Return True if the filename matches required type.
        Arguments:
            fp <str>: Path to file.
            feat_type <str>: {'fke', 'rifke'}.
        '''
        m = None
        fn = os.path.basename(fp)
        if feat_type == 'fke':
            m = re.match('(\d{5})_fke.csv', fn)
        elif feat_type == 'rifke':
            m = re.match('(\d{5})_rifke.csv', fn)
        else:
    ...
    for tup in os.walk(dir_name):
        for filename in tup[2]:
            # AC Edit: if filename.split('.')[-1] == feats_kind_dict[feats_kind]:
            if is_file_feat_type(filename, feats_kind):
